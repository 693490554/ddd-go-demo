...
优点：更加符合面向对象的原则，model的行为和数据内聚在了一起，便于管理。
缺点：model层会稍微重一些，service层很薄，service层大多数时候可能仅仅充当faced的角色。
...
缺点：model层相当重，可扩展性比较差，随着业务复杂性的增加，维护成本可能越来越高。
 - 总结 
上面的四种模型各有利弊，那么在实际项目中我们该如何做选择呢？大多数情况下失血模型和胀血模型我们是不会去考虑的，因为这两种模型都有一个共同的缺点，就是代码的分布比较失衡。那么剩下的两种模型我们该如何选择呢？个人认为在一些简单的业务场景中可以使用贫血模型，例如后台系统，基本就是对数据库的CRUD操作。一些复杂的业务场景可以使用充血模型，更加面向对象的编程有利于代码的后续迭代及维护，这些仅代表我个人观点。在实际开发过程中，大家可以根据业务情况灵活的来选择模型。
